...
* `account-admin` role on AWS `sandbox` account. Ensure it by running
...
```
aws-vault exec sso-sandbox-account-admin -- pulumi destroy -s <your_name>-ecs-test
```
...
```
# You need to have a DD APIKey in variable DD_API_KEY
aws-vault exec sso-sandbox-account-admin -- pulumi up -c scenario=aws/dockervm -c ddinfra:aws/defaultKeyPairName=<your_exisiting_aws_keypair_name> -c ddinfra:env=aws/sandbox -c ddagent:apiKey=$DD_API_KEY -c ddinfra:aws/defaultPrivateKeyPath=$HOME/.ssh/id_rsa -s <your_name>-docker
```
...
```
# You need to have a DD APIKey in variable DD_API_KEY
aws-vault exec sso-sandbox-account-admin -- pulumi up -c scenario=aws/ecs -c ddinfra:aws/defaultKeyPairName=<your_exisiting_aws_keypair_name> -c ddinfra:env=aws/sandbox -c ddagent:apiKey=$DD_API_KEY -s <your_name>-ecs
```
...
aws-vault exec sso-sandbox-account-admin -- pulumi up -c scenario=aws/eks -c ddinfra:aws/defaultKeyPairName=<your_exisiting_aws_keypair_name> -c ddinfra:env=aws/sandbox -c ddagent:apiKey=$DD_API_KEY -c ddagent:appKey=$DD_APP_KEY -s <your_name>-eks
```