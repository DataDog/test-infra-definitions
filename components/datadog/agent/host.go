...
func (h *HostAgent) installAgent(env *config.CommonEnvironment, params *agentparams.Params, baseOpts ...pulumi.ResourceOption) error {
	...
	allInstallResources := make([]pulumi.Resource, 0)
	...
	for _, input := range []struct{ path, content string }{
		...
		updateCommand, content, err := h.updateConfig(env, input.path, input.content, params.ExtraAgentConfig, utils.MergeOptions(baseOpts, utils.PulumiDependsOn(installCmd))...)
		if err != nil {
			return err
		}

		configFiles[input.path] = content
		allInstallResources = append(allInstallResources, updateCommand)
	}
	...
	_, err = h.restartAgent(
		h.namer.ResourceName("restart-agent"),
...
func (h *HostAgent) restartAgent(resourceName string, triggers pulumi.ArrayInput, opts ...pulumi.ResourceOption) (*remote.Command, error) {
...
func (h *HostAgent) installIntegrationsAndFiles(
	...
	for filePath, fileDef := range integrations {
		...
		cmd, err := h.writeFileDefinition(fullPath, fileDef.Content, fileDef.UseSudo)
		if err != nil {
			return nil, "", err
		}
