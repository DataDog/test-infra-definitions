...
func (am *agentLinuxManager) getInstallCommand(version agentparams.PackageVersion) (string, error) {
	if version.PipelineID != "" {
		...
		return fmt.Sprintf(
			`curl -L https://s3.amazonaws.com/dd-agent/scripts/%v --retry 3 -o install-script.sh && for i in 1 2 3; do DD_API_KEY=%%s %v bash install-script.sh  && break || sleep 2; done`,
