...
// K8sAppDefinitionOpenShift creates only UDP deployments for OpenShift (no UDS due to SCC restrictions)
func K8sAppDefinitionOpenShift(e config.Env, kubeProvider *kubernetes.Provider, namespace string, statsdPort int, opts ...pulumi.ResourceOption) (*componentskube.Workload, error) {
	...
	// Only create UDP deployments for OpenShift (no UDS due to hostPath volume restrictions)
