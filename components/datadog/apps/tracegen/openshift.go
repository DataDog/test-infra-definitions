...
// K8sAppDefinitionOpenShift creates only TCP deployments for OpenShift (no UDS due to SCC restrictions)
func K8sAppDefinitionOpenShift(e config.Env, kubeProvider *kubernetes.Provider, namespace string, opts ...pulumi.ResourceOption) (*componentskube.Workload, error) {
	...
	// Only create TCP deployment for OpenShift (no UDS due to hostPath volume restrictions)
