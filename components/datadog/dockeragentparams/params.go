...
func NewParams(e config.Env, options ...Option) (*Params, error) {
	...
	if e.PipelineID() != "" && e.CommitSHA() != "" {
		options = append(options, WithFullImagePath(utils.BuildDockerImagePath("669783387624.dkr.ecr.us-east-1.amazonaws.com/agent", fmt.Sprintf("pipeline-%s-%s", e.PipelineID(), e.CommitSHA()))))
	}
