...
func NewDatadogAgentInstallation(e config.CommonEnvironment, kubeProvider *kubernetes.Provider, namespace string, valueFiles []string) (*helm.Release, error) {
	...
	// Install Helm chart
	return NewHelmInstallation(e, kubeProvider, DatadogHelmRepo, "datadog", "dd", namespace, defaultAgentValues, valueFiles, pulumi.DependsOn([]pulumi.Resource{ns, secret}))
}

func NewHelmInstallation(e config.CommonEnvironment, kubeProvider *kubernetes.Provider, repoURL, chartName, installName, namespace string, inlineValues pulumi.MapInput, valueFiles []string, opts ...pulumi.ResourceOption) (*helm.Release, error) {
