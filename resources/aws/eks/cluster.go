...
func NewCluster(e aws.Environment, name string, opts ...pulumi.ResourceOption) (*Cluster, error) {
	cluster := &Cluster{}
	if err := e.Ctx.RegisterComponentResource("dd:aws:eksCluster", name, cluster, opts...); err != nil {
		return nil, err
	}

	// Create Cluster SG
	clusterSG, err := ec2.NewSecurityGroup(e.Ctx, e.Namer.ResourceName("eks", name), &ec2.SecurityGroupArgs{
		NamePrefix:  e.CommonNamer.DisplayName(pulumi.String("eks-sg")),
	...
	// Cluster role
	clusterRole, err := GetClusterRole(e, "eks-cluster-role")
	if err != nil {
		return nil, err
	}

	// IAM Node role
	linuxNodeRole, err := GetNodeRole(e, "eks-linux-node-role")
	if err != nil {
		return nil, err
	}

	windowsNodeRole, err := GetNodeRole(e, "eks-windows-node-role")
	if err != nil {
		return nil, err
	}
	...
	awsEks.NewCluster(e.Ctx, e.Namer.ResourceName(name), &awsEks.ClusterArgs{
		...
		VpcConfig: awsEks.ClusterVpcConfigArgs{
			ClusterSecurityGroupId: clusterSG.,
