...
func NewGKECluster(env gcp.Environment, opts ...Option) (*kubeComp.Cluster, error) {
	...
	return components.NewComponent(&env, env.Namer.ResourceName("gke-"+env.Ctx().Stack()), func(comp *kubeComp.Cluster) error {
		cluster, kubeConfig, err := gke.NewCluster(env, env.Ctx().Stack(), params.autopilot)
		if err != nil {
			return err
		}
